<template>
    <v-app :theme="userStore.userAll?.user?.theme?.mode || 'light'" class="cv-app">
        <div v-if="getData" :style="{ direction: lang === 'en' ? 'ltr' : 'rtl' }"
            class="fill-height d-flex flex-column">
            <!-- Desktop Header / Navbar -->
            <v-app-bar v-if="!isMobile" :elevation="isScrolled ? 4 : 0" :color="isScrolled ? 'surface' : 'transparent'"
                class="px-4 transition-all" :class="{ 'glass-header': isScrolled }" height="80" fixed>
                <!-- Logo / Name -->
                <div class="d-flex align-center cursor-pointer logo-transition" @click="scrollTo('#h-hero')"
                    :class="{ 'opacity-100': isScrolled, 'opacity-90': !isScrolled }">
                    <v-avatar v-if="userStore.userAll.user.profile_image" :image="userStore.userAll.user.profile_image"
                        size="50" class="me-3 border" :class="{ 'elevation-2': isScrolled }"></v-avatar>
                    <div>
                        ...
                        <div class="text-caption"
                            :class="{ 'text-grey-lighten-1': !isScrolled && isHeroDark, 'text-medium-emphasis': isScrolled || !isHeroDark }">
                ...
                <!-- Desktop Navigation -->
                <div class="d-flex gap-2 nav-container pa-1 rounded-pill">
                    <v-btn v-for="(item, i) in menuItems" :key="i" @click="scrollTo(item.href)"
                        :variant="activeSection === item.key ? 'tonal' : 'text'" rounded="pill"
                        class="text-capitalize font-weight-bold px-4 transition-colors"
                        :color="activeSection === item.key ? 'primary' : ''" :class="{
            ...
            <v-navigation-drawer v-model="drawer" :location="lang === 'en' ? 'left' : 'right'" temporary width="280"
                v-if="isMobile" class="glass-drawer">
                <div class="d-flex flex-column h-100 pa-4">
                    <!-- Mobile Drawer Header -->
                    <div class="text-center mb-6 mt-4">
                        <v-avatar v-if="userStore.userAll.user.profile_image"
                            :image="userStore.userAll.user.profile_image" size="100" class="mb-3 elevation-5 border-lg"
                    ...
                    <v-list nav density="comfortable" class="flex-grow-1">
                        <v-list-item v-for="(item, i) in menuItems" :key="i"
                            @click="scrollTo(item.href); drawer = false" :title="item.text" :prepend-icon="item.icon"
                            rounded="xl" :active="activeSection === item.key" color="primary"
                            class="mb-2 font-weight-medium"></v-list-item>
                    </v-list>
                </div>
            </v-navigation-drawer>
            ...
            <v-main class="bg-background flex-grow-1 position-relative">
                ...
                <div id="h-main" class="d-flex flex-column h-100">
                    <!-- Hero Section -->
                    <section id="h-hero" class="section" v-if="userStore.userAll.user.theme.hero?.enabled"
                    ...
                    <div class="content-container pa-4 pa-md-8 mx-auto w-100" style="max-width: 1200px;">
                        <section v-for="section in sections" :key="section.id" :id="section.id"
                            class="section mb-16 scroll-mt-24" v-show="section.enabled"
            ...
            <v-footer class="bg-surface text-center d-flex flex-column py-12 border-t">
                <div class="d-flex justify-center gap-4 mb-6 flex-wrap" v-if="userStore.userAll.socials">
                    <v-btn v-for="(social, i) in userStore.userAll.socials" :key="i" :href="social.url" target="_blank"
                        variant="flat" icon size="large" :color="social.icon.color || 'surface-variant'"
            ...
            <v-dialog max-width="500" v-model="dialogNoteUser" persistent>
                <v-card class="pa-6 text-center rounded-xl">
                    ...
                    <v-card-title class="text-h5 mb-2">{{ userId }}</v-card-title>
            ...
            <v-dialog v-model="dialogHelp" max-width="600">
                <v-card class="rounded-xl pa-2">
                    ...
                    <v-card-text>
                        ...
                        <v-checkbox v-model="checkboxHent" label="لا تظهر هذا التلميح مرة أخرى" color="primary"
...
// Universal Param Extraction (Supports [...path] and legacy [lang]/[id])
const pathParams = computed(() => {
    if (route.params.path && Array.isArray(route.params.path)) {
        return route.params.path;
    }
    return [];
});

// Lang: explicitly from param OR first element of path (if path has at least lang+id)
const lang = computed(() => {
    if (route.params.lang) return route.params.lang;
    if (pathParams.value.length >= 2) return pathParams.value[0];
    return 'en';
});

// ID: explicitly from param OR second element of path
const userId = computed(() => {
    if (route.params.id) return route.params.id;
    // For Root Catch-All [...path]: /ar/h -> path=['ar','h'] -> id=path[1]
    if (pathParams.value.length >= 2) return pathParams.value[1];
    return null;
});

// Debug logs
console.log("CVPage Mounted");
...
const getBasePath = () => {
    // Construct base path from computed values to be universal
    return `/${lang.value}/${userId.value}`;
};

// Centralized Side Effect: Update URL when Active Section Changes
watch(activeSection, (newSection) => {
    const sectionName = newSection === 'hero' ? '' : `/${newSection}`;
    const newPath = `${getBasePath()}${sectionName}`;
...
const scrollTo = (selector) => {
    ...
    if (el) {
        ...
        const key = selector.replace('#h-', '').replace('contact', 'contacts');
        activeSection.value = key.replace('s', '') === 'contact' ? 'contacts' : key;
        ...
        if (mapReverse[selector]) activeSection.value = mapReverse[selector];
    }
}
        ...
        .then(res => {
            ...
            nextTick(() => {
                setTimeout(() => {
                    ...
                    const possibleSection = pathParts[pathParts.length - 1];
                    ...
                    const mapReverse = {
                        ...
                        'services': '#h-services',
                        'skills': '#h-skills',
                        'academics': '#h-academic',
                        'experiences': '#h-experience',
                        'work': '#h-work',
                        'blog': '#h-blog',
                        'contacts': '#h-contact'
                    };

                    if (mapReverse[possibleSection]) {
                        scrollTo(mapReverse[possibleSection]);
                    }
                }, 300);
            });
        })
