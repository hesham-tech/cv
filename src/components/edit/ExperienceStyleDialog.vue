<template>
  <BaseEditDialog
    ...
    title="تعديل تنسيقات قسم الخبرات"
    icon="mdi-palette"
...
const originalTheme = computed(() => userStore.userAll?.user?.theme?.experiences)
...
const handleReset = async () => {
  ...
  try {
    const payload = {
      experiences: { ...originalTheme.value, title: null, text: null }
    }
    await axios.patch(`users/${userStore.user.id}/theme`, payload)
    await userStore.fetchUser()
